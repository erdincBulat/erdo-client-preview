...
class Erdo_Client_Preview_Feedback_List extends WP_List_Table {

	private Erdo_Client_Preview_DB $db;

	public function __construct( Erdo_Client_Preview_DB $db ) {
		parent::__construct( array(
			'singular' => 'erdo_client_preview_feedback',
			'plural'   => 'erdo_client_preview_feedbacks',
			'ajax'     => false,
		) );
		$this->db = $db;
	}
	...
	public function get_columns(): array {
		return array(
			...
			'author'  => __( 'Name', 'erdo-client-preview' ),
			'message' => __( 'Feedback', 'erdo-client-preview' ),
	...
	protected function column_cb( $item ): string {
		return sprintf( '<input type="checkbox" name="feedback_ids[]" value="%d" />', (int) $item->id );
	}
	...
	protected function column_author( $item ): string {
		$delete_url = wp_nonce_url(
			add_query_arg(
				array(
					'action'      => 'erdo_client_preview_feedback_delete',
					'feedback_id' => $item->id,
					'tab'         => 'feedback',
				),
				admin_url( 'admin.php?page=erdo-client-preview' )
			),
			'erdo_client_preview_feedback_delete_' . $item->id
		);
		...
		$row_actions = array(
			'delete' => sprintf(
				...
				esc_url( $delete_url ),
				esc_js( __( 'Are you sure you want to delete this feedback?', 'erdo-client-preview' ) ),
				__( 'Delete', 'erdo-client-preview' )
			),
		);

		return esc_html( $item->author_name ) . $this->row_actions( $row_actions );
	}
	...
	protected function column_status( $item ): string {
		...
		$toggle_url = wp_nonce_url(
			add_query_arg(
				array(
					'action'      => 'erdo_client_preview_feedback_status',
					'feedback_id' => $item->id,
					'status'      => $next_status,
					'tab'         => 'feedback',
				),
			...
			'erdo_client_preview_feedback_status_' . $item->id
		);
	...
	protected function column_reply( $item ): string {
		return sprintf(
			'<div class="erdo-client-preview-feedback-reply" data-item-id="%1$d">
	...
	public function prepare_items(): void {
		...
		$this->set_pagination_args( array(
			'total_items' => $this->db->get_feedback_count(),
			'per_page'    => $per_page,
		) );
		...
		$this->items = $this->db->get_feedback_paginated( $current_page, $per_page );
	}
}
